        ...
        success: function (data) {
          ...
          if (data.data != '') {
            ...
            $.each(data.data ,function(key, image) {
              console.log(image)
              if (key == 0){
                var get_room_image = "<div class='col-md-4'><div class='col-md-12 mt-25'><span title='' class=' sqr-ft sqr-ft_"+key+"'>500sqft</span> <span class='edit-icon edit-check-div_"+key+"'><img src='assets/edit-icon.png'> </span> <img class='move-type' src='"+image.image+"'><label for='room_image"+image.id+"' class='full-label'></label><div class='check-div'><div class='ins-image-align'><input id='room_image"+image.id+"' type='checkbox' class='new-app-ins item"+image.id+"' name='movesize' value='"+image.name+"' data-value='"+image.id+"' ><label>"+image.name+"</label></div></div></div></div>";
...
$(document).on('click', '.start-move-done-ok', function(){
  ...
  else if(window.location.hostname == "www.moober.com"){
    window.location.href = "http://www.moober.com/"
  }
});
...
$(document).on('click', '.payment-updated', function(){
  ...
  if (referrer === "http://localhost:3000/moves/proposal/accept" || referrer === "http://stage.moober.com/moves/proposal/accept" || referrer === "http://moober.com/moves/proposal/accept" || referrer === "http://www.moober.com/moves/proposal/accept") {
    window.location.href = "/moves/proposal/accept"
  }else {
